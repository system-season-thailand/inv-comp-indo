...
/* async function deletePastMonthRows() {
    try {
        ...
        // Filter rows where the month is 3 or more months in the past
        const rowsToDelete = allRows.filter(row => {
            ...
            // Only delete if the row's month is 3 or more months before current month
            return rowMonthIndex !== undefined && monthDifference >= 3;
        });
...
function processInvoiceData(data) {
    ...
    const createTotalPriceRow = (total, travelAgency, guestByRow) => {
        ...
        if (agencyUpper.includes("AL EZZ") || agencyUpper.includes("AL FAKHAMAH") || agencyUpper.includes("ALFAKHAMAH")) {
            currency = "USD";
